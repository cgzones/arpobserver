...
static void process_entry(const struct shm_log_entry *e, _unused_ void *arg)
{
	...
	log_info("%" PRIu64 " %s %u %s %s %s", e->timestamp, e->interface, e->vlan_tag, mac_str, ip_str, pkt_origin_str[e->origin]);
}

static void usage(void)
{
	printf("Usage: " SYSLOG_ARGV0 " [OPTIONS]\n"
	       "Forward arpobsever address events to syslog.\n\n"
